...
Read `.agents/skills/lumen-decks/SKILL.md` when creating or editing a deck. Read `.agents/skills/wcag/SKILL.md` before asserting any colour pair is accessible. Read `docs/EXTENDING.md` when adding templates, themes, fonts, brands or features. `schemas/deck.schema.json` is the content contract. `config/sources.json` is the editable research catalog.
...
node scripts/wcag.mjs audit all
```
